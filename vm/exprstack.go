...
func NewNumericVarExpression(v sframe.Variable) ExprNode {
...
// for the pair.
func NewPairVarExpression(xpart sframe.Variable, ypart sframe.Variable) ExprNode {
...
func (e *ExprNode) GetConstPair() (arithm.Pair, bool) {
	if e.IsValid() && e.IsPair {
		...
		tracer().Debugf("expression is not constant: %s", e)
	}
...
type ExprStack struct {
	stack    []ExprNode              // a stack of expressions
	leq      *polyn.LinEqSolver      // we need a system of linear equations
	resolver map[int]sframe.Variable // used to resolve variable names from IDs
}
...
func NewExprStack() *ExprStack {
	est := &ExprStack{
		stack:    make([]ExprNode, 0, 128),
		leq:      polyn.CreateLinEqSolver(),
		resolver: make(map[int]sframe.Variable),
	}
...
// fallen out of scope.
//
func (es *ExprStack) AnnounceVariable(v sframe.Variable) {
	tracer().P("var", v.Name()).Debugf("announcing id=%d", v.ID)
...
func (es *ExprStack) PushConstant(c float64) *ExprStack {
	...
	tracer().Debugf("pushing constant = %g", c)
...
func (es *ExprStack) PushPairConstant(pc arithm.Pair) *ExprStack {
	...
	tracer().Debugf("pushing pair constant = %s", e.String())
	return es.Push(e)
}
...
func (es *ExprStack) PushOtherConstant(o interface{}) *ExprStack {
	e := ExprNode{Other: o}
	tracer().Debugf("pushing tyoeless constant = %v", e)
	return es.Push(e)
}
...
//
func (es *ExprStack) PushVariable(v sframe.Variable, w sframe.Variable) *ExprStack {
	...
	if w != nil {
		...
		tracer().P("var", symname).Debugf("pushing %s", e.String())
		return es.Push(e)
	}
	tracer().P("var", v.Name).Debugf("pushing p = %s", p.String())
...
//
func (es *ExprStack) PushPairVariable(XPart sframe.Variable, xconst float64, YPart sframe.Variable,
...
// (see PushVariable(v)).
//
func (es *ExprStack) GetVariable(e *ExprNode) sframe.Variable {
	if e.IsValid() {
...
func (es *ExprStack) Dump() {
	tracer().P("size", es.Size()).Debugf("Expression Stack, TOS first:")
	for i, e := range es.stack {
		tracer().P("#", i).Debugf("    %s", e.XPolyn.TraceString(es))
	}
}
...
func (es *ExprStack) LengthTOS() error {
	...
	if !e.IsPair || !isconstx || !isconsty {
		tracer().P("op", "length").Errorf("argument must be known pair")
		return fmt.Errorf("not implemented: length(<unknown>)")
	}
	tracer().P("op", "length").Debugf("length of (%s,%s)", cx, cy)
	x := cx
	y := cy
	l := math.Sqrt(x*x + y*y)
	es.PushConstant(l)
...
func (es *ExprStack) AddTOS2OS() error {
	...
	if e1.IsPair {
		if !e2.IsPair {
			tracer().Errorf("type mismatch: <pair> + <numeric>")
		...
		if e2.IsPair {
			tracer().Errorf("type mismatch: <numeric> + <pair>")
	...
	//es.Push(&ExprNode{p})
	es.Push(e)
	tracer().P("op", "ADD").Debugf("result %s", e.String())
	return nil
}
...
func (es *ExprStack) SubtractTOS2OS() error {
	...
	//es.Push(&ExprNode{p})
	es.Push(e)
	tracer().P("op", "SUB").Debugf("result %s", e.String())
	return nil
}
...
func (es *ExprStack) MultiplyTOS2OS() error {
	...
	if e1.IsPair {
		if e2.IsPair {
			tracer().Errorf("one multiplicant must be a known numeric")
			tracer().Errorf("not implemented: <pair> * <pair>")
		} else {
			n := e2.XPolyn
			nn := n.CopyPolynomial()
	...
	es.Push(e)
	tracer().P("op", "MUL").Debugf("result = %s", e.String())
	return nil
}
...
func (es *ExprStack) DivideTOS2OS() error {
	...
	e1, _ = es.Pop()
	if e2.IsPair {
		tracer().Errorf("divisor must be a known non-zero numeric")
	...
	es.Push(e)
	tracer().P("op", "DIV").Debugf("result = %s", e.String())
	return nil
}
...
func (es *ExprStack) Interpolate() (err error) {
	if err = es.CheckOperands(2, "interpolate"); err == nil {
		...
		if a.IsPair {
			...
			es.Push(e)
			tracer().P("op", "INTERP").Debugf("result = %s", p.String())
		}
	}
	return
}
...
//
func (es *ExprStack) InterpolatePair(n, z1, z2 ExprNode) error {
	// second operand will be destroyed, n must be first !
	...
	e := NewPairExpression(px, py)
	es.Push(e)
	tracer().P("op", "INTERP").Debugf("result = %s", e.String())
	return nil
}
...
func (es *ExprStack) Rotate2OSbyTOS() error {
	...
	tracer().Debugf("sin %s° = %s, cos %s° = %s", c, sin, c, cos)
	e, _ = es.Pop()
	tracer().Debugf("rotating %v by %s° = %f rad", e, c, angle)
	tracer().Errorf("TODO: rotation calculation is buggy")
	if e.IsPair {
		...
		tracer().P("op", "rotate").Errorf("not implemented: rotate <non-pair>")
